...
public interface ObjectRetrievalResponseMapper {
    ...
    @Mapping(target = "key", source = "userObject.objectKey")
    @Mapping(target = "checksum", source = "userObject", qualifiedByName = "checksum")
    RetrieveObjectResponse map(UserObject userObject, InputStream inputStream);

    @Named("checksum")
    default Checksum mapChecksum(UserObject userObject) {
